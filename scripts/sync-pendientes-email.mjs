...
function esFacturaPropia(acreedor) {
  const a = (acreedor || "").toUpperCase();
  return (
    a.includes("MARTINEZ GUEDES") ||
    a.includes("MARTÍNEZ GUEDES") ||
    a.includes("IGNACIO JAVIER MARTINEZ") ||
    a.includes("IGNACIO MARTINEZ GUEDES") ||
    a.includes("IGNACIO JAVIER MARTÍNEZ")
    );
}
    ...
    messages: [{
      ...
      content: [
        contentBlock,
        {
          type: "text",
          text:
            `Estás filtrando adjuntos de mail para decidir si son una factura que la empresa ` +
            `"Martínez Guedes" (Ignacio Javier Martínez Guedes, despachante de aduanas) tiene ` +
            `que PAGAR ella misma — es decir, un gasto propio de la empresa.\n\n` +
            `NO aplica (respondé {"aplica":false}) en estos casos, aunque el documento sea una factura real:\n` +
            `- Fue EMITIDA por Ignacio Javier Martínez Guedes o por "Martínez Guedes" (es una factura que ELLOS le mandan a un cliente, no algo que deban pagar).\n` +
            `- Es una factura COMERCIAL de un proveedor del exterior hacia un cliente de Martínez Guedes ` +
            `(ej: proveedores como DAMM, IPANCO, TEXTIL JADUE, o cualquier otro exportador extranjero — ` +
            `esto es documentación de la operación de importación de un cliente, no un gasto de Martínez Guedes).\n` +
            `- No es claramente una factura o comprobante de pago a nombre de Martínez Guedes como responsable de pagar.\n` +
            `- Cualquier caso dudoso: ante la duda, respondé {"aplica":false}. Es preferible no incluirla a incluir algo que no corresponde.\n\n` +
            `Si SÍ aplica (es un gasto/factura que Martínez Guedes debe pagar), respondé SOLO con este JSON, sin texto extra ni backticks:\n` +
            `{"aplica":true,"acreedor":"nombre empresa/proveedor que emite la factura","monto":número_total,"moneda":"peso o usd","vencimiento":"YYYY-MM-DD o null","confianza":"alta|media|baja"}\n` +
            `Si NO aplica, respondé SOLO: {"aplica":false}\n` +
...
try {
  ...
  for await (const msg of client1.fetch(`${lastUid + 1}:${maxUidActual}`, { uid: true, envelope: true, bodyStructure: true }, { uid: true })) {
    ...
    if (/cierre\s+cont[aá]ble/i.test(asunto)) {
      continue;
    }
  ...
  try {
    ...
    for (const meta of cand.candidatos) {
      ...
      if (analysis?.aplica && analysis?.acreedor && analysis?.monto && !esPropia) {
        ...
        console.log(`  ⏭️  "${meta.filename}" parece emitida por Martínez Guedes (no es un gasto propio) — se salta`);
      } else {
        console.log(`  ⚠️  "${meta.filename}" no es un pendiente a pagar (o la IA no pudo extraer los datos) — se salta`);
      }
    }
    } finally {
